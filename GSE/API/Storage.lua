...
local GNOME = "Storage"
local VARIABLE_SELFKEY_PREFIX = "GSEVar_"  -- self-key prefix for variable event handlers
...
function GSE.OOCUpdateSequence(name, sequence)
    ...
    if GSE.RefreshActionBarOverrideIcons then
        ...
        if GSE.After then
            GSE.After(0, function() GSE.RefreshActionBarOverrideIcons(name, false) end)
            GSE.After(0.1, function() GSE.RefreshActionBarOverrideIcons(name, false) end)
            GSE.After(0.25, function() GSE.RefreshActionBarOverrideIcons(name, false) end)
        end
    end
...
function GSE.GetCurrentButtonIconInfo(self, reseticon)
    ...
    elseif action.type == "item" then
        local mname, _, _, _, _, _, _, _, _, micon = GSE.GetItemInfo(GSE.UnEscapeString(action.item))
        if mname then
            spellinfo.name = mname
            spellinfo.iconID = micon
            foundSpell = spellinfo.name
...
local function GetDebuggerTraceSpell(action, foundSpell)
    ...
    if action.type == "item" and action.item then
        local itemName = GSE.GetItemInfo(GSE.UnEscapeString(action.item))
