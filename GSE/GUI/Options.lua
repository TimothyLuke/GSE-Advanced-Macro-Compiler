...
function GSE.GetOptionsTable()
  ...
  for k,v in pairs(GSEOptions.AddInPacks) do
    ...
    OptionsTable.args.pluginsTab.args[v.Name] = {
      name = v.Name,
      desc = string.format(L["Addin Version %s contained versions for the following macros:"], L["Addin Version %s contained versions for the following macros:"])..string.format(["\n%s"], GSE.FormatSequenceNames(v.SequenceNames)),
      type = "execute",
      func = function(info, val)
