...
GSE.GUI.SyncTrees = function()
    ...
    if shower ~= gui.lastTreeShower then
        ...
        elseif tc and tc.RevealSelection then
            if GSE.After then
                GSE.After(0, function() if tc and tc.RevealSelection then tc:RevealSelection() end end)
            else
                tc:RevealSelection()
            end
        end
...
local function getItemIconInfo(candidate)
    local icon = select(10, GSE.GetItemInfo(candidate))
    if icon then
        return {
            name = candidate,
            iconID = icon,
...
local function getActionAutoIcon(action)
    ...
    elseif actionType == "item" or actionType == "toy" then
        ...
        if itemKey then
            return notQuestionMark(select(10, GSE.GetItemInfo(itemKey)))
        end
...
function GSE.CreateIconControl(action, version, keyPath, sequence, frame)
    ...
    lbl:SetCallback("OnClick", function(widget, button)
        GSE.CreateContextMenu(frame, function(ownerRegion, rootDescription)
...
function GSE.CreateEditor()
	...
	editframe:SetCallback(
		...
		function(self)
            ...
            if GSE.After and GSE.GUI.ClearUndoIfNoVisibleEditors then
                GSE.After(0, GSE.GUI.ClearUndoIfNoVisibleEditors)
            end
            self.Sequence = nil
    ...
    local function expandEditorFromWidget(self)
        ...
        if self.editframe then
            ...
            if self.editframe.leftPanel and self.leftPanelWasVisible then
                ...
                restorePanel()
                if GSE.After then
                    GSE.After(0,    restorePanel)
                    GSE.After(0.15, restorePanel)
                end
            end
        end
    ...
    editframe.frame:HookScript("OnHide", function()
        ...
        GSE.After(0, RefreshEditorActivation)
    end)
    ...
    local function DrawSequenceEditor(tcontainer, version, path)
        ...
        local function SelectMacroBlockPath(keyPath, overlay, force)
            ...
            if editframe._gseFocusSelectedMacroBlock then
                GSE.After(0.05, editframe._gseFocusSelectedMacroBlock)
            end
        end
                    ...
                    if v ~= editframe.Sequence.MetaData.Name then
                        ...
                        if specid then
                            if sid ~= specid and (sid or 0) > 13 and specid > 13 then
                                local val = select(2, GSE.GetSpecializationInfoByID(specid))
    ...
    local function GUIDrawMacroEditor(container, version, path)
        ...
        local function InsertTopToolbarAction(newAction)
            ...
            if insertedPath and FocusSelectedMacroBlock then
                GSE.After(0.01, function()
                    if editframe._gseSelectMacroBlockPath then
                        editframe._gseSelectMacroBlockPath(finalInsertedPath, nil, true)
                    end
                    FocusSelectedMacroBlock()
                end)
                GSE.After(0.06, function()
                    if editframe._gseSelectMacroBlockPath then
                        editframe._gseSelectMacroBlockPath(finalInsertedPath, nil, true)
                    end
                    FocusSelectedMacroBlock()
                end)
                GSE.After(0.18, function()
                    if editframe._gseSelectMacroBlockPath then
                        editframe._gseSelectMacroBlockPath(finalInsertedPath, nil, true)
                    end
                    FocusSelectedMacroBlock()
                end)
                GSE.After(0.45, function()
        ...
        local function RefocusSelectedMacroBlock()
            ...
            FocusSelectedMacroBlock()
            GSE.After(0.01, FocusSelectedMacroBlock)
            GSE.After(0.05, FocusSelectedMacroBlock)
        end
        ...
        function BlinkSelectedMacroBlock()
            ...
            setBlinkAlpha(0.18)
            GSE.After(0.08, function() setBlinkAlpha(1) end)
            GSE.After(0.16, function() setBlinkAlpha(0.18) end)
            GSE.After(0.24, function() setBlinkAlpha(1) end)
        end
...
function GSE.ShowSequences()
    if not InCombatLockdown() or (GSE.PlayerSpellsLoaded and GSE.PlayerSpellsLoaded()) then
        ...
        if lastArea and lastArea ~= "Sequences" and editframe.RestoreLastNode then
            GSE.After(0.1, function() editframe.RestoreLastNode() end)
        end
