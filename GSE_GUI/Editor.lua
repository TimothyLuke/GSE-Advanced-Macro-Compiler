...
function GSE.CreateEditor()
    ...
    local function GUIDrawMetadataEditor(container)
        ...
        scrollcontainer:SetHeight(editframe.Height - 150)
        ...
        DrawTalentsEditor(contentcontainer)

        container:AddChild(scrollcontainer)
    end

    local function ChooseVersion(version, scrollpos, path)
    ...
    local function drawAction(container, action, version, keyPath, treepath)
            ...
            if GSE.isEmpty(disableMove) then
                ...
                moveUpButton:SetCallback(
                    ...
                    function()
                        ...
                        ChooseVersion(version, editframe.scrollStatus.scrollvalue, treepath)
                    end
                )
                moveUpButton:SetCallback(
                    "OnEnter",
    ...
    local function DrawSequenceEditor(container, version, path)
    ...
    local function GUIDrawMacroEditor(container, version, path)
        ...
        scrollcontainer:SetFullWidth(true)
        scrollcontainer:SetHeight(editframe.Height - 220)
        scrollcontainer:SetLayout("Fill") -- Important!
        ...
        addActionButton:SetCallback(
            ...
            function()
                ...
                ChooseVersion(version, editframe.scrollStatus.scrollvalue, path)
            end
        )
        addActionButton:SetCallback(
            "OnEnter",
                ...
                elseif mbutton == "LeftButton" and IsShiftKeyDown() then
                    ...
                    elseif not GSE.isEmpty(sequencename) then
                        ...
                        elseif key == "config" then
                            GUIDrawMetadataEditor(rightContainer)
                        elseif key == "newversion" then
                            table.insert(
                                editframe.Sequence.Macros,
                                GSE.CloneSequence(editframe.Sequence.Macros[editframe.Sequence.MetaData.Default])
                            )
                            treeContainer:SelectByValue(group .. "\001" .. tostring(#editframe.Sequence.Macros))
                            return
                        else
                            local path = GSE.CloneSequence(unique)
                            table.remove(path, #path)
                            GUIDrawMacroEditor(rightContainer, key, table.concat(path, "\001"))
                        end
