...
remoteFrame:SetCallback("OnClose", function(widget)  remoteFrame:Hide() end)
...
function GSE.ShowRemoteWindow(SequenceList, GSEUser)
  ...
  for ClassID,v in ipairs(remoteFrame.SequenceList) do
    ...
    if ClassID > 0 then
      ...
      classbutton:SetImage(GSE.GetClassIcon(ClassID))
      linegroup1:AddChild(classbutton)
    end
    local classLabel = AceGUI:Create("Label")
    classLabel:SetText(Statics.SpecIDList[ClassID])
    ...
    for name, value in pairs(v) do
      ...
      addKeyPairRow(contentcontainer, columnWidth, name, desc, ClassID)
    end
  end
