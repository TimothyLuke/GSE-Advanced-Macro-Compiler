...
function GSE.TraceSequence(button, step, spell, blockPath)
    if GSE.UnsavedOptions.DebugSequenceExecution then
        ...
        local isUsable, notEnoughMana = GSE.IsSpellUsable(spell)
        ...
        if GSE.GameMode > 5 then
            if spellid then
                FoundInSpellBook = GSE.FindSpellBookSlotForSpell(spellid)
...
function GSE:UNIT_SPELLCAST_SUCCEEDED(event, unit, action, sped)
    if unit == "player" then
        ...
        if GSE.GameMode > 10 then
            ...
            local found = GSE.FindSpellBookSlotForSpell(elements[6])
            if found then
                foundskill = true
