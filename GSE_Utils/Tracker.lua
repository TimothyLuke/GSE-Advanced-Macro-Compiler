...
local function SetActiveSpamKey(sequence, keyText)
    ...
    After(Statics.TrackerConfig.ActiveSpamKeyHoldSeconds + 0.02, function()
...
local function ResolveActionIcon(action)
    ...
    elseif action.type == "item" and action.item then
        ...
        local itemName, _, _, _, _, _, _, _, _, itemIcon = GSE.GetItemInfo(item)
...
local function ScheduleSequenceIconRefresh()
    ...
    After(0, RefreshAllSequenceIcons)
    After(0.25, RefreshAllSequenceIcons)
    After(1, RefreshAllSequenceIcons)
end
