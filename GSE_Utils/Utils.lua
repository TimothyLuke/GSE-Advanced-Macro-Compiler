...
function GSE.OOCPerformMergeAction(action, classid, sequenceName, newSequence)
    ...
    -- Every new and imported sequence lands here, and REPLACE writes into the
    -- Library directly rather than through ReplaceSequence -- so the stamp on
    -- save never sees it, and it went without a key until the next load.
    -- Stamp it under the name it is stored as; an existing stamp is kept.
