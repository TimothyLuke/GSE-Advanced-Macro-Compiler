                                ...
                                for bidx, action in ipairs(macroversion.Actions) do
                                    if action.Type == "Action" and action.type ~= "pet" and not action.Icon then
                                        ...
                                        elseif action.type == "item" or action.type == "toy" then
                                            ...
                                            hasIcon = key and select(10, GSE.GetItemInfo(key)) ~= nil
...
function GSE.PrintGnomeHelp()
    ...
    GSE.Print(
        GSEOptions.CommandColour ..
            L[
                "GSE also registers these standalone commands: /gseresettracker (restore the tracker to its default layout), /gsesavelayoutx and /gsesavelayouty (save the current tracker layout to slot X or Y), /gseapplylayoutx and /gseapplylayouty (apply a saved layout), /gseiconscan and /gsespelliconreset and /gsesaveallsequences (action-icon maintenance), and /rl (reload the UI)."
            ],
        GNOME
    )
end
...
function GSE:GSSlash(input)
    ...
    if command == "showspec" then
        if GSE.GameMode < 7 then
            ...
            local _, specname, _, _, _, _, _ = GSE.GetSpecializationInfoByID(currentSpecID)
...
do
    ...
    local function gseEmptyButtonHandler(self, mousebutton, down)
        ...
        local classIconText = ""
        local classInfo = GSE.GetClassInfo(GSE.GetCurrentClassID())
        ...
        GSE.CreateContextMenu(self, function(ownerRegion, rootDescription)
            ...
            for _, entry in ipairs(names) do
                ...
                if specID and specID >= 15 then
                    local _, _, _, specIconID = GSE.GetSpecializationInfoByID(specID)
                    if specIconID then
