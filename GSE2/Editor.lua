...
editframe:SetTitle(L["GSE2 Retro:"] .. " " .. L["Sequence Editor"])
...
editframe.frame:SetScript(
    ...
    function(self, width, height)
        ...
        GSEOptions.editorHeight = editframe.Height
        GSEOptions.editorWidth = editframe.Width
        GSE2.GUISelectEditorTab(editframe.ContentContainer, "Resize", editframe.SelectedTab)
        editframe:DoLayout()
    end
)
...
function GSE2.GUIEditorPerformLayout(frame)
    ...
    tabgrp:SetTabs(GSE2.GUICreateEditorTabs())
    editframe.ContentContainer = tabgrp

    tabgrp:SetCallback(
        "OnGroupSelected",
        function(container, event, group)
            GSE2.GUISelectEditorTab(container, event, group)
        end
    )
    ...
    savebutton:SetCallback(
        ...
        function()
            ...
            editframe.SequenceName = nameeditbox:GetText()
            local GSE3sequence = GSE.ConvertGSE2(editframe.Sequence, editframe.SequenceName)
            GSE.GUIUpdateSequenceDefinition(editframe.ClassID, editframe.SequenceName, GSE3sequence)
        end
    )
    ...
    savebutton:SetCallback(
        "OnEnter",
        function()
            GSE.CreateToolTip(L["Convert"], L["Convert this to a GSE3 Template"], editframe)
        end
    )
    savebutton:SetCallback(
        "OnLeave",
...
function GSE2:GUIDrawMetadataEditor(container)
...
function GSE2:GUIDrawMacroEditor(container, version)
    ...
    editframe.Sequence.MacroVersions[version] =
        GSE.TranslateSequence(editframe.Sequence.MacroVersions[version], "From Editor", "CURRENT")
    ...
    if not GSE.isEmpty(editframe.Sequence.MacroVersions[version].PostMacro) then
        PostMacro:SetText(GSE.SafeConcat(editframe.Sequence.MacroVersions[version].PostMacro, "\n"))
    end
    PostMacro:SetCallback(
        "OnTextChanged",
        function(sel, object, value)
    ...
    showBaseSpells:SetCallback(
        "OnValueChanged",
        function(sel, object, value)
            GSEOptions.showCurrentSpells = value
            GSE2.GUISelectEditorTab(container, "event", version)
        end
    )
    showBaseSpells:SetCallback(
        "OnEnter",
    ...
    if
        not GSE.isEmpty(editframe.Sequence.MacroVersions[version].LoopLimit) and
            tonumber(editframe.Sequence.MacroVersions[version].LoopLimit) > 0
     then
    ...
    if not GSE.isEmpty(editframe.Sequence.MacroVersions[version].KeyPress) then
        KeyPressbox:SetText(GSE.SafeConcat(editframe.Sequence.MacroVersions[version].KeyPress, "\n"))
    end
    KeyPressbox:SetCallback(
        "OnTextChanged",
        function(sel, object, value)
    ...
    PreMacro.editBox:SetScript(
        "OnLeave",
        function()
            GSE.GUIParseText(PreMacro)
        end
    )
    PreMacro:SetCallback(
        "OnEnter",
        function()
            GSE.CreateToolTip(
                L["PreMacro"],
    ...
    if not GSE.isEmpty(editframe.Sequence.MacroVersions[version].PreMacro) then
        PreMacro:SetText(GSE.SafeConcat(editframe.Sequence.MacroVersions[version].PreMacro, "\n"))
    end
    PreMacro:SetCallback(
        "OnTextChanged",
        function(sel, object, value)
    ...
    if not GSE.isEmpty(editframe.Sequence.MacroVersions[version]) then
        spellbox:SetText(GSE.SafeConcat(editframe.Sequence.MacroVersions[version], "\n"))
    end
    spellbox:SetCallback(
        "OnTextChanged",
        function(sel, object, value)
    ...
    KeyReleasebox.editBox:SetScript(
        "OnLeave",
        function()
            GSE.GUIParseText(KeyPressbox)
        end
    )
    KeyReleasebox:SetCallback(
        "OnEnter",
        function()
            GSE.CreateToolTip(
                L["KeyRelease"],
    ...
    if not GSE.isEmpty(editframe.Sequence.MacroVersions[version].KeyRelease) then
        KeyReleasebox:SetText(GSE.SafeConcat(editframe.Sequence.MacroVersions[version].KeyRelease, "\n"))
    end
    KeyReleasebox:SetCallback(
        "OnTextChanged",
        function(sel, object, value)
...
function GSE2:GUIDrawVariableEditor(container)
...
local function addKeyPairWARow(container, rowWidth, key, value)
    ...
    loadWeakAuraButton:SetCallback(
        "OnClick",
        function()
            GSE2.LoadWeakAura(valueEditBox:GetText())
        end
    )
    loadWeakAuraButton:SetCallback(
        "OnEnter",
...
function GSE2:GUIDrawWeakauraStorage(container)
    if GSE.isEmpty(editframe.Sequence.Variables) then
        editframe.Sequence.Variables = {}
    end
...
function GSE2.GUISelectEditorTab(container, event, group)
    if not GSE.isEmpty(container) then
        container:ReleaseChildren()
        editframe.SelectedTab = group
        editframe.nameeditbox:SetText(GSE2.GUIEditFrame.SequenceName)
        editframe.iconpicker:SetImage(GSE.GetMacroIcon(editframe.ClassID, editframe.SequenceName))
        if group == "config" then
            GSE2:GUIDrawMetadataEditor(container)
        elseif group == "new" then
            -- Copy the Default to a new version
            table.insert(
                editframe.Sequence.MacroVersions,
                GSE.CloneSequence(editframe.Sequence.MacroVersions[editframe.Sequence.Default])
            )

            GSE2.GUIEditorPerformLayout(editframe)
            GSE2.GUISelectEditorTab(container, event, table.getn(editframe.Sequence.MacroVersions))
        elseif group == "variables" then
            GSE2:GUIDrawVariableEditor(container)
        elseif group == "weakauras" then
            GSE2:GUIDrawWeakauraStorage(container)
        else
            GSE2:GUIDrawMacroEditor(container, group)
        end
    end
end

function GSE2.GUIDeleteVersion(version)
    version = tonumber(version)
    ...
    GSE2.GUIEditFrame:SetStatusText(string.format(printtext, version))
end
