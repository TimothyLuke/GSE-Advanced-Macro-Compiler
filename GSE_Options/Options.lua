...
local ABOUT_LOGO_TEXTURE = "Interface\\AddOns\\GSE_GUI\\Assets\\GSE-Logo.png"
...
local function OpenBlizzardColorPickerForPopup(popup)
    ...
    if GSE.After then GSE.After(0, function() HookModernColorPickerClassButton(applyClassColor) end) end
end
...
local function AddActionBarClickBehaviorOptions(optionsCategory)
    ...
    do
        local function GetValue()
            local value = GSE.GetCVar and GSE.GetCVar("ActionButtonUseKeyDown")
            return tonumber(value) == 1
        end
        local function SetValue(val)
            local ok, err = pcall(function()
                if GSE.SetCVar then
                    return GSE.SetCVar("ActionButtonUseKeyDown", val and 1 or 0)
                end
            end)
            if not ok then
    ...
    do
        ...
        setting:SetValueChangedCallback(function()
            if GSEOptions.SyncWoWMacros and GSE.SyncAllWoWMacros then
                GSE.After(0, GSE.SyncAllWoWMacros)
            end
        end)
    end
end
    ...
    do
        ...
        if GSE.isEmpty(GSE.AddInPacks) then
            ...
            for _, v in pairs(GSE.AddInPacks) do
                ...
                local displayName = GSE.GetAddOnMetadata(packName, "Title") or packName
                local desc = GSE.GetAddOnMetadata(packName, "Notes") or
